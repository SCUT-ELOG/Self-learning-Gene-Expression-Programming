/***************************************
Source code of SL-GEP.
Related paper:
J.Zhong, Y.-S. Ong, and W. Cai. "Self-learning gene expression programming," IEEE Transactions on Evolutionary Computation, 20.1 (2016): 65-80.
Version: 1.0
Last update: 2017-03-15
Coded by£º Jinghui Zhong
*/
...
void read_data(int run)
{
	...
	char name[200];
	i = (int) (run / 10);
	sprintf(name, "symbolic regression dataset\\F%d_%d_training_data.txt",function, i);
	f = fopen(name,"r");
	int row_num, col_num;
	fscanf(f, "%d\t%d\n", &row_num, &col_num);
